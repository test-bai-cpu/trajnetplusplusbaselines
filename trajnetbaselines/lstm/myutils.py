...
# edin
# Because pixel-2-meter is also done when generating the dataset for prediction. So don't need to change here.
def read_edin_human_traj_data_by_person_id(human_traj_file, person_id):
    data = pd.read_csv(human_traj_file, header=None)
    data.columns = ["time", "person_id", "x", "y", "velocity", "motion_angle"]
    # data = pixel_to_meter(data, ["x", "y"])
    human_traj_data = data.loc[data['person_id'] == person_id]
    human_traj_array = human_traj_data[["time", "x", "y", "velocity", "motion_angle"]].to_numpy()

    return human_traj_array
# edin
def read_edin_human_traj_data(human_traj_file):
    data = pd.read_csv(human_traj_file, header=None)
    data.columns = ["time", "person_id", "x", "y", "velocity", "motion_angle"]
    # data = pixel_to_meter(data, ["x", "y"])
...
# standford drone
def read_drone_human_traj_data_by_person_id(human_traj_file, person_id):
    data = pd.read_csv(human_traj_file, header=None)
    data.columns = ["time", "person_id", "x", "y", "velocity", "motion_angle"]
    human_traj_data = data.loc[data['person_id'] == person_id]
    human_traj_array = human_traj_data[["time", "x", "y", "velocity", "motion_angle"]].to_numpy()
    ...
    return human_traj_array
# standford drone
def read_drone_human_traj_data(human_traj_file):
    data = pd.read_csv(human_traj_file, header=None)
    data.columns = ["time", "person_id", "x", "y", "velocity", "motion_angle"]
    return data
...
# the cliff-map still needs the pixel to meter. Because we train cliff-map with pixels, not in meters of x and y.
def read_edin_cliff_map_data(cliff_map_file):
    data = pd.read_csv(cliff_map_file, header=None)
    data.columns = ["x", "y", "motion_angle", "velocity",
                    "cov1", "cov2", "cov3", "cov4", "weight",
                    "motion_ratio", "observation_ratio"]
...
def read_stef_full_map_data(stef_map_file):
    data = pd.read_csv(stef_map_file, header=None)
    data.columns = ["x", "y", "theta_1", "theta_2", "theta_3", "theta_4", "theta_5", "theta_6", "theta_7", "theta_8"]
    return data.to_numpy()

def read_edin_stef_full_map_data(stef_map_file):
    data = pd.read_csv(stef_map_file, header=None)
    data.columns = ["x", "y", "theta_1", "theta_2", "theta_3", "theta_4", "theta_5", "theta_6", "theta_7", "theta_8"]
    data = pixel_to_meter(data, ["x", "y"])
    return data.to_numpy()

def read_stef_uni_map_data(stef_map_file):
    data = pd.read_csv(stef_map_file, header=None)
